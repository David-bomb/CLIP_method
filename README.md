...
Теперь мы можем загрузить фото! При первой попытке загрузить фотографию для поиска нам придется подождать загрузку и инициализацию модели, это займет некоторое время. Но потом все будет проходить быстро. В целом за одну сессию использования браузера можно перезапускать проект и модели будут сохраняться. Если вы хотите вручную удалить модели из памяти, то зайдите в режим разработчика => `Application` => `Storage` => `Clear Site Data`. После этого модели будут загружаться заново. 
